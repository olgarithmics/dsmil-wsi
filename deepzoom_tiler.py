...
def nested_patches(img_slide, out_base, level=(0,), ext='jpeg'):
    ...
    n_levels = len(glob.glob('/data/scratch/DBI/DUDBI/DYNCESYS/OlgaF/tmi/colon/WSI_temp_files/*'))
    bag_path = os.path.join(out_base, img_class, img_name)
    os.makedirs(bag_path, exist_ok=True)
    if len(level)==1:
        patches = glob.glob(os.path.join('/data/scratch/DBI/DUDBI/DYNCESYS/OlgaF/tmi/colon/WSI_temp_files', '*', '*.'+ext))
        ...
        levels = [int(os.path.basename(i)) for i in glob.glob(os.path.join('/data/scratch/DBI/DUDBI/DYNCESYS/OlgaF/tmi/colon/WSI_temp_files', '*'))]
        levels.sort()
        low_patches = glob.glob(os.path.join('/data/scratch/DBI/DUDBI/DYNCESYS/OlgaF/tmi/colon/WSI_temp_files', str(levels[0]), '*.'+ext))
        for i, low_patch in enumerate(low_patches):
            ...
            for x_pos in high_x_list:
                for y_pos in high_y_list:
                    high_patch = glob.glob(os.path.join('/data/scratch/DBI/DUDBI/DYNCESYS/OlgaF/tmi/colon/WSI_temp_files', str(levels[1]), '{}_{}.'.format(x_pos, y_pos)+ext))
                    if len(high_patch)!=0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='colon', help='Dataset name')
    ...
    path_base = os.path.join('/data/scratch/DBI/DUDBI/DYNCESYS/OlgaF/slides/', args.dataset)
    if len(levels) == 2:
    ...
    for idx, c_slide in enumerate(all_slides):
        ...
        DeepZoomStaticTiler(c_slide, '/data/scratch/DBI/DUDBI/DYNCESYS/OlgaF/tmi/colon/WSI_temp', levels, args.base_mag, args.objective, args.format, args.tile_size, args.overlap, True, args.quality, args.workers, args.background_t).run()
        nested_patches(c_slide, out_base, levels, ext=args.format)
        shutil.rmtree('/data/scratch/DBI/DUDBI/DYNCESYS/OlgaF/tmi/colon/WSI_temp_files')
